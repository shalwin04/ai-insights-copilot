...
export const driveApi = {
  ...
  downloadFile: (fileId: string) =>
    api.get(`/api/drive/files/${fileId}/download`, {
      responseType: "blob",
    }),
  searchFiles: (query: string) =>
    api.get("/api/drive/search", {
      params: { query },
    }),
};
...
export const explorerApi = {
  ...
  getDatasetOverview: (datasetId: string) =>
    api.get(`/api/explorer/datasets/${datasetId}/overview`),
  generateInsights: (datasetId: string) =>
    api.post(`/api/explorer/datasets/${datasetId}/insights`),
  ...
  getColumnDetails: (datasetId: string, columnName: string) =>
    api.get(
      `/api/explorer/datasets/${datasetId}/columns/${encodeURIComponent(
        columnName
      )}`
    ),
};
